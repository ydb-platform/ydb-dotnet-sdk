...
// TODO: Success rate ~500/550
public class ComplexNavigationsSharedTypeQueryYdbTest
    : ComplexNavigationsSharedTypeQueryRelationalTestBase<ComplexNavigationsSharedTypeQueryYdbFixture>
{
    public ComplexNavigationsSharedTypeQueryYdbTest(ComplexNavigationsSharedTypeQueryYdbFixture fixture) : base(fixture)
    {
    }

    [ConditionalTheory(Skip = "TODO: Correlated subquery")]
    ...
    public override Task Order_by_key_of_navigation_similar_to_projected_gets_optimized_into_FK_access(bool async) =>
        base.Order_by_key_of_navigation_similar_to_projected_gets_optimized_into_FK_access(async);

    [ConditionalTheory(Skip = "OrderBy parameter not included in SELECT")]
    [MemberData(nameof(IsAsyncData))]
    public override Task
        Order_by_key_of_projected_navigation_doesnt_get_optimized_into_FK_access_subquery(bool async) =>
        base.Order_by_key_of_projected_navigation_doesnt_get_optimized_into_FK_access_subquery(async);

    [ConditionalTheory(Skip = "OrderBy parameter not included in SELECT")]
    [MemberData(nameof(IsAsyncData))]
    public override Task
        Order_by_key_of_anonymous_type_projected_navigation_doesnt_get_optimized_into_FK_access_subquery(bool async) =>
        base.Order_by_key_of_anonymous_type_projected_navigation_doesnt_get_optimized_into_FK_access_subquery(async);
    ...
    [MemberData(nameof(IsAsyncData))]
    public override Task
        Projection_select_correct_table_from_subquery_when_materialization_is_not_required(bool async) =>
        base.Projection_select_correct_table_from_subquery_when_materialization_is_not_required(async);

    [ConditionalTheory(Skip = "OrderBy parameter not included in SELECT")]
    [MemberData(nameof(IsAsyncData))]
    public override Task Projection_select_correct_table_with_anonymous_projection_in_subquery(bool async) =>
        base.Projection_select_correct_table_with_anonymous_projection_in_subquery(async);

    [ConditionalTheory(Skip = "OrderBy parameter not included in SELECT")]
    [MemberData(nameof(IsAsyncData))]
    public override Task
        Projection_select_correct_table_in_subquery_when_materialization_is_not_required_in_multiple_joins(
            bool async) =>
        base.Projection_select_correct_table_in_subquery_when_materialization_is_not_required_in_multiple_joins(async);

    [ConditionalTheory(Skip = "OrderBy parameter not included in SELECT")]
    [MemberData(nameof(IsAsyncData))]
    public override Task Where_predicate_on_optional_reference_navigation(bool async) =>
        base.Where_predicate_on_optional_reference_navigation(async);

    [ConditionalTheory(Skip = "OrderBy parameter not included in SELECT")]
    [MemberData(nameof(IsAsyncData))]
    public override Task GroupJoin_on_a_subquery_containing_another_GroupJoin_projecting_outer(bool async) =>
        base.GroupJoin_on_a_subquery_containing_another_GroupJoin_projecting_outer(async);

    [ConditionalTheory(Skip = "OrderBy parameter not included in SELECT")]
    [MemberData(nameof(IsAsyncData))]
    public override Task GroupJoin_on_a_subquery_containing_another_GroupJoin_projecting_inner(bool async) =>
        base.GroupJoin_on_a_subquery_containing_another_GroupJoin_projecting_inner(async);

    [ConditionalTheory(Skip = "OrderBy parameter not included in SELECT")]
    [MemberData(nameof(IsAsyncData))]
    public override Task
        GroupJoin_on_a_subquery_containing_another_GroupJoin_with_orderby_on_inner_sequence_projecting_inner(
            bool async) =>
        base.GroupJoin_on_a_subquery_containing_another_GroupJoin_with_orderby_on_inner_sequence_projecting_inner(
            async);
    ...
    public override Task Where_on_multilevel_reference_in_subquery_with_outer_projection(bool async) =>
        base.Where_on_multilevel_reference_in_subquery_with_outer_projection(async);
