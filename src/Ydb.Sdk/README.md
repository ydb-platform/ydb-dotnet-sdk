...
ADO metrics (meter `Ydb.Sdk.Ado`): operation histogram/counter use `database`, `endpoint`, `operation.name` (and `status_code` on failures); pool metrics use `ydb.query.session.pool.name`. The metrics `AddYdb()` subscribes to both `Ydb.Sdk.Ado` and `Ydb.Sdk.Topic`; use `AddYdbAdo()` or `AddYdbTopic()` for selective subscription. See [README.md](../Ydb.Sdk.OpenTelemetry/README.md).
