...
- Feat: per-call client component chain in the `x-ydb-sdk-build-info` header (matches the Python SDK `sdk-name/version;component/version` format). ADO.NET sessions automatically append `;ado-net/{SDK_VERSION}`; Topic readers and writers append `;topic-reader/{SDK_VERSION}` and `;topic-writer/{SDK_VERSION}`. Higher-level frameworks (e.g. EFCore) can extend the ADO.NET chain via the new internal `YdbConnectionStringBuilder.ClientInfo` init-only property — when set, the resulting header becomes `ydb-dotnet-sdk/{V};ado-net/{V};{ClientInfo}`. Internal SDK calls (Discovery, schema operations, bulk upsert, static credentials login) continue to send the bare `ydb-dotnet-sdk/{SDK_VERSION}` value.
