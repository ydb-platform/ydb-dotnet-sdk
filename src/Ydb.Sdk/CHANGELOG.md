...
- Feat Topic Reader metrics: added four counters and a partition-session gauge on the `Ydb.Sdk.Topic` meter.
  ...
  All Topic Reader metrics have the `endpoint`, `database`, `consumer`, and `reader.name` attributes; counters also
  have `topic`. The partition-session gauge is an SDK-local snapshot, not server ownership or processing progress.
