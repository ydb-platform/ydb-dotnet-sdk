...
- Feat tracing: added `ydb.Login` span emitted by the static-credentials auth client. `StaticCredentialsAuthClient.FetchToken` now drives its retries through `YdbRetryPolicyExecutor` with `operationName: "ydb.Login"`, so each login attempt produces an outer `ydb.Login` span with nested `ydb.Try` children.
- **Breaking Change**: Removed legacy `Retry.cs` (`BackoffSettings`, `RetryPolicy` enum, `RetryRule`, `RetrySettings`). All call sites (Topic `Reader<T>` / `Writer<T>` initialization and `StaticCredentialsAuthClient`) have been migrated to `IRetryPolicy` / `YdbRetryPolicyExecutor`.
