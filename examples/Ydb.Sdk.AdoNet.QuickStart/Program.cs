...
await using var ydbDataSource = new YdbDataSource("Host=localhost;Port=2136;Database=/local");
...
async Task SelectWithParameters()
{
...
async Task CreatingUser()
{
    ...
    logger.LogInformation("Creating user with password: [user, password]");
    await ydbCommand.ExecuteNonQueryAsync();
    logger.LogInformation("Created user");
}

async Task InteractiveTransaction()
{
    await using var ydbConnection = new YdbConnection(_cmdOptions.SimpleConnectionString);
    await ydbConnection.OpenAsync();
    ...
    logger.LogInformation("Updated episode title: {}", ydbCommand.ExecuteScalar());
}
...
async Task TlsConnectionExample()
{
    ...
    while (await ydbDataReader.ReadAsync())
    {
        logger.LogInformation("season_title: {SeasonTitle}, series_title: {SeriesTitle}, " +
...
async Task ReadResultSetsWithUserPassword()
{
    ...
    while (await ydbDataReader.NextResultAsync())
    {
        while (await ydbDataReader.ReadAsync())
        {
            logger.LogInformation("Iterate by resultSets: {}", ydbDataReader.GetValue(0));
        }
    }
}
