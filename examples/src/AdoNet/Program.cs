...
async Task Run(CmdOptions cmdOptions)
{
    await InitTables(cmdOptions);
    await LoadData(cmdOptions);
}

async Task InitTables(CmdOptions o)
{
    // YdbConnection from connection string
    await using var connection = new YdbConnection(o.ConnectionString);
...
async Task LoadData(CmdOptions o)
{
...
async Task SelectWithParameters(CmdOptions o)
{
    await using var connection = new YdbConnection(o.ConnectionString);
    await connection.OpenAsync();
