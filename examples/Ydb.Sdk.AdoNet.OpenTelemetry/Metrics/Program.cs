...
if (args.Length > 0 && args[0] == "--load")
{
    ...
    var totalSec = args.Length > 1 && int.TryParse(args[1], out var s) ? s : 600;
    ...
    Console.WriteLine(
        $"=== YDB Metrics Load Tank ({totalSec}s, {workerCount} workers) ===\n" +
    ...
    var controller = Task.Run(async () =>
    {
                           ...
                           totalSec, token))
        {
            ...
            var stepToken = CancellationTokenSource
                .CreateLinkedTokenSource(token, currentRpsCts.Token).Token;
...
await ds.ExecuteAsync(async conn =>
{
    ...
    await new YdbCommand("CREATE TABLE demo_inserts(id Uuid, val Int32, PRIMARY KEY (id))", conn)
